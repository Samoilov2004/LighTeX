...
for path in sorted(args.metadata_directory.glob("*.metadata.json")):
    ...
    if archive_parts:
        item["downloadParts"] = [
            {"downloadUrl": f"{args.release_base_url.rstrip('/')}/{part['archiveName']}", "compressedSize": part["compressedSize"]}
...
expected = {
    ...
    for platform, architecture in (("macOs", "arm64"), ("macOs", "x86_64"), ("linux", "x86_64"))
}
actual = {(item["variant"], item["platform"], item["architecture"]) for item in assets}
if actual != expected or len(assets) != 9:
    raise SystemExit(f"Expected nine unique runtime assets; missing={sorted(expected - actual)}, extra={sorted(actual - expected)}")

manifest = {"schemaVersion": 2, "runtimeVersion": args.runtime_version, "texLiveYear": args.texlive_year, "assets": assets}
